package com.riverssen.nucleus.ui;
...
import com.riverssen.nucleus.crypto.MnemonicPhraseSeeder;
import com.riverssen.nucleus.crypto.ec.ECLib;
import com.riverssen.nucleus.exceptions.*;
...
import com.riverssen.nucleus.util.FileUtils;
...
public class Wallet extends Application implements Initializable
{
    ...
    com.riverssen.nucleus.crypto.Wallet wallet;
    ...
    void start(String ... args) throws Throwable
    {
        ...
        double        maxDifficulty = com.riverssen.nucleus.system.Parameters.MAXIMUM_DIFFICULY;
...
//                        FileUtils.writeBytesRAW(entryPoint.newFile("GeoLiteC").newFile("GeoLiteC.mmdb").file(), GeoLiteC_mmdb);
        ...
        context = new NucleusContext(entryPoint, db, true);
    }
    ...
    public void onOpenExistingWallet(ActionEvent event)
    {
        ...
        try
        {
            wallet = new com.riverssen.nucleus.crypto.Wallet(fileService, password);
//            chain.read(password, (DataInputStream) fileService.as(DataInputStream.class));
    ...
    public void onStep3Next(ActionEvent event)
    {
        ...
        if (step2_wordlist.getText().replaceAll("\\s+", " ").replaceAll("\\n", " ").equals(step3_wordlist.getText().replaceAll("\\s+", " ").replaceAll("\\n", " ")))
        {
            ...
            try
            {
                ...
                wallet = new com.riverssen.nucleus.crypto.Wallet(file, password, seeder.getSeed());
    ...
    public void onCopyPrivateKeyButtonPressed(ActionEvent event) throws Throwable
    {
        ...
        Toolkit.getDefaultToolkit().getSystemClipboard().setContents(new StringSelection(com.riverssen.nucleus.crypto.Wallet.WIFPrivateKey(wallet.getPrivateKey(password))), null);
    }
    ...
    public void onAddRecipientButtonPressed(ActionEvent event) throws Throwable
    {
        ...
        BigDecimal decimal = new BigDecimal(amount).multiply(new BigDecimal(com.riverssen.nucleus.system.Parameters.SATOSHIS_PERCOIN));

        if (decimal.longValue() < com.riverssen.nucleus.system.Parameters.MINIMUM_TRANSACTION)
            return;

        amount = new BigDecimal(decimal.longValue()).divide(new BigDecimal(com.riverssen.nucleus.system.Parameters.SATOSHIS_PERCOIN), 11, BigDecimal.ROUND_UP).toPlainString();
    ...
    private void sendTransaction(boolean genNewKeypair)
    {
        ...
        long total = 0;
        long fee = com.riverssen.nucleus.system.Parameters.coinToSatoshis(feepmb_field.getText());
        ...
        for (int i = 0; i < recipient_list.getItems().size(); i ++)
        {
            ...
            outputs[i].setValue(longval = new BigDecimal(amount).multiply(BigDecimal.valueOf(com.riverssen.nucleus.system.Parameters.SATOSHIS_PERCOIN)).longValue());
        ...
        long actualFee = totalBytes * fee;//new BigDecimal(totalBytes).divide(new BigDecimal(1000000), 34, BigDecimal.ROUND_UP).multiply(new BigDecimal(com.riverssen.nucleus.system.Parameters.PRICE_PER_NETWORK_MBYTE)).max(new BigDecimal(com.riverssen.nucleus.system.Parameters.PRICE_PER_NETWORK_MBYTE))
        ...
        Alert alert = new Alert(Alert.AlertType.CONFIRMATION, "This transaction has a total cost of \"" + com.riverssen.nucleus.system.Parameters.satoshisToCoin(total + fee).toPlainString() + "\" coins, continue?\n" +
                "Breakdown:\n\tTotal: " + com.riverssen.nucleus.system.Parameters.satoshisToCoin(total).toPlainString() +
                "\n\tFees: " + com.riverssen.nucleus.system.Parameters.satoshisToCoin(fee).toPlainString()
