...
int create_address_from_string(const unsigned char *string,
    ...
    u_int8_t * hash = (u_int8_t *) malloc(SHA256_DIGEST_LENGTH);
    BIGNUM * n = BN_new();
    ...
    u_int8_t * ripemd = (u_int8_t *) malloc(RIPEMD160_DIGEST_LENGTH+1+4);
    ...
    u_int8_t * buf = (u_int8_t *) malloc(buflen+1+4);
    int datalen;
...
void prepare_for_address(unsigned char *data, int datalen, char start_byte) {
    unsigned char *tmpbuf = (unsigned char *) malloc(datalen);
